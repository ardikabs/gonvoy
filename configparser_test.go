...
func TestConfigParser(t *testing.T) {
	...
	t.Run("filter config unset, fallback to map | Parent and Child", func(t *testing.T) {
		...
		mergedInternalConfig, ok := mergedCfg.(*globalConfig)
		assert.True(t, ok)
	...
	t.Run("with filter config | Parent and Child", func(t *testing.T) {
		...
		assert.Same(t, parentCfg.(*globalConfig).internalCache, mergedInternalConfig.internalCache)
		assert.Same(t, mergedInternalConfig.internalCache, childCfg.(*globalConfig).internalCache)
		assert.Same(t, parentCfg.(*globalConfig).callbacks, mergedInternalConfig.callbacks)
		assert.Same(t, mergedInternalConfig.callbacks, childCfg.(*globalConfig).callbacks)
		assert.NotSame(t, parentCfg.(*globalConfig).filterConfig, mergedInternalConfig.filterConfig)
		assert.Same(t, mergedInternalConfig.filterConfig, childCfg.(*globalConfig).filterConfig)
	})
